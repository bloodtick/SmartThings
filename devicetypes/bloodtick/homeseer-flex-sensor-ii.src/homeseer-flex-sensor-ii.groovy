/**
 *  HomeSeer Flex Sensor II
 *
 *  Original Author: 
 *    Kevin LaFramboise (krlaframboise)
 *
 *  Modified by HomeSeer for HS-FS100+ Flex Sensor
 *
 ...
 *
 * Version 1.1  2/25/19
 * Updated 1.2  9/10/19 Bloodtick - added Contact Sensor, Switch, Actuator. Changed color and fixed multiAttributeTile. Turned off temp notifications. Pref title mods.
 */
metadata {
    definition (
        name: "HomeSeer Flex Sensor II", 
        namespace: "bloodtick", 
        author: "SmartThings"
    ) {
        capability "Sensor"	
    ...
    tiles(scale: 2) {
        multiAttributeTile(name:"switch", type: "lighting", width: 6, height: 4, canChangeIcon: false){
            tileAttribute ("device.switch", key: "PRIMARY_CONTROL") {
                ...
                attributeState "nowater", 
                    label:'No Water', 
                    icon: "st.alarm.water.dry",
                    backgroundColor:"#ffffff"               
                attributeState "water", 
                    label:'Water', 
        ...
        main "switch"
        details(["switch", "temperature", "contact", "battery", "refresh"])
    }
}
...
def configure() {
    ...
    if (!state.isConfigured) {
        ...
        sendEvent(getEventMap("switch", "nolight", false))		
        //cmds << "delay 1000"
    }
...
def zwaveEvent(physicalgraph.zwave.commands.notificationv3.NotificationReport cmd) {
    ...
    if (cmd.notificationType == 0x5) {
        // water
        switch (cmd.event) {
            case 0:
                logDebug "Sensor No Water"				
                result << createEvent(getEventMap("switch", "nowater"))
                result << createEvent(name: "contact", value: "open", descriptionText: "Contact is open: No Water")
                break
            case 2:
                logDebug "Sensor Detects Water"
                result << createEvent(getEventMap("switch", "water"))
                result << createEvent(name: "contact", value: "closed", descriptionText: "Contact is closed: Water")
                break
        }
    ...
    if (cmd.notificationType == 0x14) {
        switch (cmd.event) {
            case 0:
                ...
                result << createEvent(getEventMap("switch", "off"))
                result << createEvent(name: "contact", value: "open", descriptionText: "Contact is open: No Light")
                break
            case 1:
                logDebug "Sensor Detects Light"
                result << createEvent(getEventMap("switch", "on"))
                result << createEvent(name: "contact", value: "closed", descriptionText: "Contact is closed: Light")
                break
            case 2:
                logDebug "Sensor Detects Color Change"
                result << createEvent(getEventMap("switch", "colorchange"))
                result << createEvent(name: "contact", value: "closed", descriptionText: "Contact is closed: Color Change")
                break
            default:
...
private handleTemperatureEvent(cmd) {
    ...
    result << createEvent(createEventMap("temperature", val, false, "Temperature ${val}${getTemperatureScale()}", getTemperatureScale()))
    return result
}
